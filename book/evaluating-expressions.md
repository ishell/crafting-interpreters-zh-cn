...
在 Lox 中，<span name="value">值</span>由字面量所创建，由表达式所计算，并被存放在变量之中。从用户视角看去，它们是 *Lox* 对象；但它们实际是由我们解释器所使用的底层语言来实现的。这意味着我们要在 Lox 动态类型的世界与 Java 静态类型的世界之间架起一座桥梁。Lox 中的一个变量能够存放任何（Lox）类型的值，甚至能在不同的时间点存放不同类型的值。我们究竟可以使用哪一种 Java 类型来表征这一切呢？
...
根据 [IEEE 754][ieee 754]——它规定了双精度浮点数的种种行为——将零除以零会得到那个特殊的**NaN**（"非数字"，Not a Number）值。颇为吊诡的是，NaN 居然**不等于**它自身。
...
眼下，倘若一个操作数的类型与正在执行的运算不相匹配，Java 的类型转换便会失败，JVM 也便会抛出一个 ClassCastException。这会一路展开整个调用栈，并退出应用程序，将一串 Java 栈跟踪一股脑儿地吐到用户脸上。这多半不是我们想要的。Lox 是由 Java 实现的这件事实，理应作为一个隐藏在幕后的细节。相反，我们希望用户能够明白：这是一条**Lox 的**运行时错误，并给出一条与我们的语言及其程序息息相关的错误消息。
...
最后，我们把[上一章][last chapter]里那段用于打印语法树的临时代码删去，换成下面这段：
...
Java 本可以通过禁止第一行的那种转型来规避这一运行时的检查。它本可以使数组成为**不变的**，从而一个 Integer 数组**便不再**是一个 Object 数组。从静态的角度来说这是稳妥的，但它会禁止那些仅从数组中读取元素的常见而安全的代码模式。倘若你从不对数组执行*写*操作，那么协变便是安全的。这些模式在 Java 1.0 那个还不支持泛型的年代，对可用性而言尤为重要。James Gosling 以及其他的 Java 设计者们牺牲了一点点静态安全性与性能——那些数组存储检查毕竟要耗时——以换取一定的灵活性。
